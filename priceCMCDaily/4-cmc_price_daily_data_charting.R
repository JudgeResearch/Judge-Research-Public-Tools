...
for (i in (1:length(coinSummary$coin))){
  for (j in (1:length(coinSummary$coin))){
    ...
    #
    postscript(file.path("./chart",paste0(cmc_coin$coin[i],"Usd",cmc_coin$coin[j],"UsdDccJan2016.eps")), fonts="serif",
...
for (i in (1:length(coinSummary$coin))){
  for (j in (1:length(coinSummary$coin))){
    ...
    #
    postscript(file.path("./chart",paste0(cmc_coin$coin[i],"Usd",cmc_coin$coin[j],"UsdDccMar2018.eps")), fonts="serif",
               width = 15, height =7.5, horizontal = TRUE, family= "serif", paper ="special")
